...
from app.services.icsp_data_fill_service import ICSP_FILL_LIST_URL
from app.services.k2_print_service import BPM_KEEPALIVE_URLS
from app.services.parking_api_sync_service import KEEPALIVE_URLS
from app.services.script_logger import build_script_logger


logger = build_script_logger("run_4pyun_browser", "4pyun_browser.log")
...
SOURCE_PROFILE_DIR = Path(
    os.getenv("PARKING_BOOTSTRAP_CHROME_PROFILE_DIR", str(Path.home() / ".config" / "google-chrome"))
)
MANAGED_REMOTE_DEBUGGING_PORT = int(os.getenv("PARKING_MANAGED_CHROME_PORT", "9223"))
MANAGED_HEADLESS = os.getenv("PARKING_MANAGED_CHROME_HEADLESS", "false").lower() == "true"
...
def bootstrap_profile() -> None:
    ...
    if SOURCE_PROFILE_DIR.exists():
        logger.info("bootstrapping managed 4pyun chrome profile from %s", SOURCE_PROFILE_DIR)
...
def build_command(env: dict[str, str]) -> list[str]:
    ...
    for url in BPM_KEEPALIVE_URLS:
        if url not in urls:
            urls.append(url)
    if ICSP_FILL_LIST_URL not in urls:
        urls.append(ICSP_FILL_LIST_URL)
    args.extend(urls)
    return args
...
def main() -> None:
    ...
    logger.info("starting managed 4pyun chrome profile=%s port=%s", MANAGED_PROFILE_DIR, MANAGED_REMOTE_DEBUGGING_PORT)
