...
DEFAULT_INPUT_DIR = Path(r"D:\python\menbers\claude\backup\parking\exports_leave_time_20250101_20260228")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import parking record CSV files into the database.")
    parser.add_argument("--input-dir", default=str(DEFAULT_INPUT_DIR), help="Directory that contains parking record CSV files.")
...
def main() -> None:
    ...
    with SessionLocal() as session:
        ...
        try:
            service = ParkingRecordCsvImportService(db=session, logger=log_callback, batch_size=args.batch_size)
